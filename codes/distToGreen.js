...
// Lista de municípios
var munCodeList = ee.List(setCens.aggregate_array('CD_MUN').distinct()).getInfo()
...
// var test = distanceToGreenAreas (munCode)
// .select(selectors)
...
// calculting the distances 
var distCalc = ee.FeatureCollection(munCodeList.map(distanceToGreenAreas))
    .flatten()
    .select(selectors)
...
var description = 'distanceToGreenAreas_v' + info.version
// + '_test'
...
Export.table.toAsset({
  collection: distCalc, 
  description: description, 
  assetId: info.assetId + '/' + description,
})
